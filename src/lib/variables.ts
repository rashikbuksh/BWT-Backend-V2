  ...
  const query = sql`
    ...
    sg_off_days AS (
      ...
      date_ranges AS (
        SELECT shift_group_uuid,
          GREATEST(effective_date, (SELECT start_date FROM params2)) AS period_start,
          LEAST(COALESCE(next_effective_date - INTERVAL '1 day', (SELECT end_date FROM params2)), (SELECT end_date FROM params2)) AS period_end,
          off_days
        FROM roster_periods
        WHERE GREATEST(effective_date, (SELECT start_date FROM params2)) <= LEAST(COALESCE(next_effective_date - INTERVAL '1 day', (SELECT end_date FROM params2)), (SELECT end_date FROM params2))
      ),
      all_days AS (
    ...
    attendance_data AS (
      ...
      LEFT JOIN hr.manual_entry me_late ON me_late.employee_uuid = e.uuid
          AND me_late.entry_time::date = d.punch_date
