...
DECLARE 
    warehouse_name TEXT;
BEGIN
    SELECT assigned INTO warehouse_name FROM store.warehouse WHERE uuid = (SELECT warehouse_uuid FROM inventory.purchase_return WHERE uuid = NEW.purchase_return_uuid);
...
-- Delete Trigger
CREATE OR REPLACE FUNCTION inventory.product_after_purchase_return_entry_delete_function() RETURNS TRIGGER AS $$
DECLARE 
    warehouse_name TEXT;
BEGIN
    SELECT assigned INTO warehouse_name FROM store.warehouse WHERE uuid = (SELECT warehouse_uuid FROM inventory.purchase_return WHERE uuid = OLD.purchase_return_uuid);
...
CREATE OR REPLACE FUNCTION inventory.product_after_purchase_return_entry_update_function() RETURNS TRIGGER AS $$
DECLARE 
    old_warehouse_name TEXT;
    new_warehouse_name TEXT;
BEGIN
    -- Get old and new warehouse names
    SELECT assigned INTO old_warehouse_name FROM store.warehouse WHERE uuid = (SELECT warehouse_uuid FROM inventory.purchase_return WHERE uuid = OLD.purchase_return_uuid);
    SELECT assigned INTO new_warehouse_name FROM store.warehouse WHERE uuid = (SELECT warehouse_uuid FROM inventory.purchase_return WHERE uuid = NEW.purchase_return_uuid);
...
AFTER INSERT ON inventory.purchase_return_entry
FOR EACH ROW
EXECUTE FUNCTION inventory.product_after_purchase_return_entry_insert_function();

CREATE OR REPLACE TRIGGER product_after_purchase_return_entry_delete_trigger
AFTER DELETE ON inventory.purchase_return_entry
FOR EACH ROW
EXECUTE FUNCTION inventory.product_after_purchase_return_entry_delete_function();

CREATE OR REPLACE TRIGGER product_after_purchase_return_entry_update_trigger
AFTER UPDATE ON inventory.purchase_return_entry
FOR EACH ROW
EXECUTE FUNCTION inventory.product_after_purchase_return_entry_update_function();