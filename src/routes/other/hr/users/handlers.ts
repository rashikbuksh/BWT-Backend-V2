...
const receivedTrue = sql`CASE WHEN 
      ...
      THEN ${engineerOrder.quantity}::float8 END`;
    ...
    const engineerQuery = db
      .select({
        ...
        label: sql`CONCAT(${hrSchema.users.name}, 
            ' (', 'WIH: ', (SUM(COALESCE(${receivedTrue}, 0))::float8 + SUM(COALESCE(${diagnosisTrue}, 0))::float8 + SUM(COALESCE(${repairTrue}, 0))::float8), ')')`,
        received_count: sql`SUM(COALESCE(${receivedTrue}, 0))::float8`,
        diagnosis_count: sql`SUM(COALESCE(${diagnosisTrue}, 0))::float8`,
        repair_count: sql`SUM(COALESCE(${repairTrue}, 0))::float8`,
      })
      .from(hrSchema.users)
